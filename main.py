...
def extract_command_result(full_output, command):
    ...
    if cmd_line_idx == -1:
        ...
        for i in range(cmd_line_idx + 1, len(lines)):
            line = lines[i]
            if re.match(r'^[\w\-_]+@[\w\-_]+.*\$\s*$', line.strip()): break
            if line.strip().endswith('$ ') and len(line.strip()) > 2: break
...
def execute_command(chat_id, command):
    ...
    if success:
        wait_time = 3
        if any(k in command.lower() for k in ['install','apt','pip','gcloud','docker','kubectl','terraform']):
            wait_time = 10
        ...
        if output_text:
            ...
            bot.send_message(chat_id,
                f"✅ تم تنفيذ: `{command}`\n📋 لم يتم التقاط النص (شاهد الصورة)",
                parse_mode="Markdown")
