...
def _generate_vless_cmd(region, token, chat_id):
    """توليد السكريبت مع دمج سكريبت التثبيت المقدم من المستخدم"""
...
# --- 💡 دمج السكريبت الخاص بك لتثبيت لوحة 3X-UI محلياً ---
echo "======================================================="
...
USERNAME=$(sudo sqlite3 /etc/x-ui/x-ui.db "SELECT value FROM settings WHERE key='username';")
PASSWORD=$(sudo sqlite3 /etc/x-ui/x-ui.db "SELECT value FROM settings WHERE key='password';")
BASEPATH=$(sudo sqlite3 /etc/x-ui/x-ui.db "SELECT value FROM settings WHERE key='webBasePath';")
...
<i>ملاحظة: ظهور شاشة خطأ 404 عند الدخول لرابط Cloud Run من المتصفح هو أمر طبيعي جداً.</i>"
